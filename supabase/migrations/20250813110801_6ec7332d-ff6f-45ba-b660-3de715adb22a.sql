-- Fix company data security issues (updated)
...
CREATE POLICY "Users can view their company diagnostic results only" 
ON public.diagnostic_results 
FOR SELECT 
USING (company_id = public.get_user_company_id());

DROP POLICY IF EXISTS "Users can insert diagnostic results" ON public.diagnostic_results;
CREATE POLICY "Users can insert their company diagnostic results only" 
ON public.diagnostic_results 
FOR INSERT 
WITH CHECK (company_id = public.get_user_company_id());

DROP POLICY IF EXISTS "Users can update diagnostic results" ON public.diagnostic_results;
CREATE POLICY "Users can update their company diagnostic results only" 
ON public.diagnostic_results 
FOR UPDATE 
USING (company_id = public.get_user_company_id());
...
CREATE POLICY "Users can view their company preventive measures only" 
ON public.preventive_measures 
FOR SELECT 
USING (company_id = public.get_user_company_id());

DROP POLICY IF EXISTS "Users can insert preventive measures" ON public.preventive_measures;
CREATE POLICY "Users can insert their company preventive measures only" 
ON public.preventive_measures 
FOR INSERT 
WITH CHECK (company_id = public.get_user_company_id());

DROP POLICY IF EXISTS "Users can update preventive measures" ON public.preventive_measures;
CREATE POLICY "Users can update their company preventive measures only" 
ON public.preventive_measures 
FOR UPDATE 
USING (company_id = public.get_user_company_id());
...
CREATE POLICY "Users can view their company SCIAN actions only" 
ON public.scian_actions 
FOR SELECT 
USING (company_id = public.get_user_company_id());

DROP POLICY IF EXISTS "Users can insert SCIAN actions" ON public.scian_actions;
CREATE POLICY "Users can insert their company SCIAN actions only" 
ON public.scian_actions 
FOR INSERT 
WITH CHECK (company_id = public.get_user_company_id());

DROP POLICY IF EXISTS "Users can update SCIAN actions" ON public.scian_actions;
CREATE POLICY "Users can update their company SCIAN actions only" 
ON public.scian_actions 
FOR UPDATE 
USING (company_id = public.get_user_company_id());
...
CREATE POLICY "Users can view their company compliance history only" 
ON public.compliance_history 
FOR SELECT 
USING (company_id = public.get_user_company_id());

DROP POLICY IF EXISTS "Users can insert compliance history" ON public.compliance_history;
CREATE POLICY "Users can insert their company compliance history only" 
ON public.compliance_history 
FOR INSERT 
WITH CHECK (company_id = public.get_user_company_id());
...
-- Note: "Users can view their own conversation logs" already exists, keeping it
