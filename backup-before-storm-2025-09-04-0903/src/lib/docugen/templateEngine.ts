...
{
  id: 'notifications_temps_reel_v1',
  name: 'Notifications temps réel non-conformités',
  version: '1.0',
  legislation: ['CNESST', 'LMRSST'],
  subject: 'alertes-temps-reel',
  targetSectors: ['tous'],
  companySize: 'all',
  priority: 'critical',
  agent: 'MonitoringAgent',
  templatePath: '/templates/notifications_nc.md.j2',
  placeholders: [
    {
      id: 'alert_type',
      name: 'Type d\'alerte',
      ...
      required: true,
      source: 'user_input',
      values: ['critique', 'majeure', 'mineure']
    }
  ],
  generationRules: [],
  qualityChecks: [],
  metadata: {
    author: 'MonitoringAgent',
    createdDate: '2025-09-01',
    version: '1.0',
    tags: ['notification', 'temps-reel'],
    estimatedGenerationTime: 30,
    outputFormats: ['json', 'notification']
  }
  ,
},
{
  id: 'diagnostic_lmrsst_v1',
  name: 'Diagnostic conformité LMRSST complet',
  version: '1.0',
  legislation: ['LMRSST'],
  subject: 'diagnostic-conformite',
  targetSectors: ['tous'],
  companySize: 'all',
  priority: 'mandatory',
  agent: 'DiagSST',
  templatePath: '/templates/diagnostic_lmrsst.md.j2',
  placeholders: [],
  generationRules: [],
  qualityChecks: [],
  metadata: {
...
export class DocuGenEngine {
  ...
  private async generateContent(
    ...
    // Ajout manuel des données pour les notifications temps réel
    if (template.id === 'notifications_temps_reel_v1') {
      templateData.company_name = request.additionalData?.company_name || request.companyProfile?.name;
      templateData.detection_timestamp = request.additionalData?.detection_timestamp;
      templateData.responsible_agent = request.additionalData?.responsible_agent;
      templateData.priority_level = request.additionalData?.priority_level;
      templateData.incident_details = request.additionalData?.incident_details;
      templateData.notification_channels = request.additionalData?.notification_channels;
      templateData.generated_at = request.additionalData?.generated_at;
    }
  ...
  private getPlaceholderValue(placeholder: any, request: DocumentGenerationRequest, legalContext?: any): any {
    switch (placeholder.source) {
      case 'user_input':
        console.log('Cherche placeholder:', placeholder.id, 'avec source:', placeholder.source);
        const result = this.extractFromProfile(placeholder.id, request.companyProfile, request.additionalData);
        console.log('Résultat pour', placeholder.id, ':', result);
        return result;
...
'alertes-temps-reel': `# SYSTÈME DE NOTIFICATION TEMPS RÉEL
## Non-conformités {{alert_type}}

**Entreprise:** {{company_name}}
**Date:** {{detection_timestamp}}
**Agent responsable:** {{responsible_agent}}
...
*Système AgenticSST Québec*`
,
'diagnostic-conformite': `# DIAGNOSTIC DE CONFORMITÉ LMRSST
**Entreprise:** {{company_name}}
**Nombre d'employés:** {{company_size}}
**Code SCIAN:** {{scian_code}}
**Date du diagnostic:** {{generated_date}}
...
**Rapport généré par:** DiagSST
*Conforme aux articles 51 et 90 de la LMRSST*`
    };
